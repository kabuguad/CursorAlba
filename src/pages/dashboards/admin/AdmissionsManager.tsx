...
import { Eye, CheckCircle, XCircle, Clock, Search, X, Download, User, Trash2, Loader2 } from 'lucide-react'
...
type AppStatus = AdmissionApplication['status']
...
export function AdmissionsManager() {
  ...
  const { data: apps = [],  isLoading } = useAdmissions()
  const { data: stats }                 = useAdmissionsStats()
  const updateStatus                    = useUpdateAdmissionStatus()
  const deleteApp                       = useDeleteAdmission()

  const [search, setSearch]       = useState('')
  const [statusFilter, setStatus] = useState<AppStatus | 'all'>('all')
  ...
  const filtered = apps.filter(a => {
    ...
    const parent = `${a.parentFirstName} ${a.parentLastName}`.toLowerCase()
    const matchSearch = name.includes(q) || parent.includes(q) || a.id.toLowerCase().includes(q)
  ...
  const counts = {
    all:       apps.length,
    pending:   stats?.pending   ?? apps.filter(a => a.status === 'pending').length,
    reviewing: stats?.reviewing ?? apps.filter(a => a.status === 'reviewing').length,
    approved:  stats?.approved  ?? apps.filter(a => a.status === 'approved').length,
  ...
  const openApp = (a: AdmissionApplication) => { setSelected(a); setNotes(a.notes) }
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {counts.all} applications · {counts.pending} pending review · Live db data
          </p>
        </div>
        <button
        ...
        {(['pending', 'reviewing', 'approved', 'rejected'] as AppStatus[]).map(s => {
          ...
          return (
            <button key={s} onClick={() => setStatus(s)}
              className={`text-left rounded-2xl border p-4 transition hover:scale-[1.01] ${
                statusFilter === s
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px] text-sm">
              <thead className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
                <tr>
                  {['Ref', 'Child', 'Grade', 'Parent', 'Submitted', 'Docs', 'Status', 'Actions'].map(h => (
                    ...
                    <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-medium text-gray-900 dark:text-white">{a.childFirstName} {a.childLastName}</p>
                            <p className="text-[10px] text-gray-400">DOB: {a.dob} · {a.gender}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3.5 text-gray-600 dark:text-gray-300">{a.applyingForGrade}</td>
                      <td className="px-5 py-3.5">
                        <p className="text-gray-900 dark:text-white">{a.parentFirstName} {a.parentLastName}</p>
          ...
          <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="font-semibold text-gray-900 dark:text-white">{selected.childFirstName} {selected.childLastName}</p>
                <p className="text-sm text-gray-400">DOB: {selected.dob} · {selected.gender}</p>
                <p className="text-sm text-gray-400">Applying for: {selected.applyingForGrade}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Parent / Guardian</p>
                <p className="font-semibold text-gray-900 dark:text-white">{selected.parentFirstName} {selected.parentLastName}</p>
            ...
            {selected.previousSchool && (
              <div>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Previous School</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">{selected.previousSchool}</p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selected.documents.length === 0 && <p className="text-sm text-gray-400">No documents uploaded</p>}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => handleUpdateStatus(selected.id, 'reviewing')} disabled={updateStatus.isPending}
                  className="rounded-xl bg-blue-500 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-600 transition disabled:opacity-50">Mark Reviewing</button>
                <button onClick={() => handleUpdateStatus(selected.id, 'approved')} disabled={updateStatus.isPending}
                  className="rounded-xl bg-green-500 px-4 py-2 text-sm font-semibold text-white hover:bg-green-600 transition disabled:opacity-50">Approve</button>
                <button onClick={() => handleUpdateStatus(selected.id, 'pending')} disabled={updateStatus.isPending}
                  className="rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition disabled:opacity-50">Reset to Pending</button>
                <button onClick={() => handleUpdateStatus(selected.id, 'rejected')} disabled={updateStatus.isPending}
                  className="rounded-xl bg-red-500 px-4 py-2 text-sm font-semibold text-white hover:bg-red-600 transition disabled:opacity-50">Reject</button>
              </div>
            </div>
