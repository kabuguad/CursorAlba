...
const BLANK: CoreValueCreateDto = { icon: '⭐', title: '', description: '', sortOrder: 1 }
...
export function CoreValuesManager() {
  ...
  const [draft, setDraft] = useState<CoreValueCreateDto>(BLANK)
  const [delConfirm, setDelConfirm] = useState<number | null>(null)

  const set = <K extends keyof CoreValueCreateDto>(k: K, v: CoreValueCreateDto[K]) =>
    setDraft(d => ({ ...d, [k]: v }))
  ...
  const openEdit = (item: CoreValue) => {
    setDraft({ icon: item.icon, title: item.title, description: item.description, sortOrder: item.sortOrder })
    setEditing(item)
    setModalOpen(true)
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Core Values</h2>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
            Values shown in the Core Values grid on the public About page.
          </p>
        </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4">
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 text-center">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Total Values</p>
        </div>
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 text-center">
          <p className="text-2xl font-bold text-green-500">{items.filter(i => i.description).length}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">With Description</p>
        </div>
      </div>
      ...
      {!isLoading && items.length === 0 && (
        ...
        >
          <Plus className="h-5 w-5" /> Add your first core value
        </button>
      )}
      ...
      {!isLoading && sorted.length > 0 && (
        <div className="space-y-2">
          {sorted.map(item => (
            <div
              key={item.id}
              className="flex items-center gap-4 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3"
            >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="What this value means to Alber School…"
            />
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-1">
            <button
              onClick={handleSave}
              disabled={createMut.isPending || updateMut.isPending || !draft.title.trim()}
              className={BTN_GOLD}
            >
              <Check className="h-3.5 w-3.5" />
              {createMut.isPending || updateMut.isPending ? 'Saving…' : editing ? 'Save Changes' : 'Add Value'}
            </button>
