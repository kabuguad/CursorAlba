...
import { Plus, Pencil, Trash2, X, Search, Clock, CheckCircle } from 'lucide-react'
...
const BLANK: Omit<SchoolEvent, 'id'> = { title: '', date: new Date().toISOString().slice(0, 10), location: '', description: '', isPast: false }
...
export function EventsManager() {
  ...
  const [evts, setEvts] = useState<SchoolEvent[]>(SEED.map(e => ({ ...e })))
  const [search, setSearch] = useState('')
  const [editing, setEditing] = useState<SchoolEvent | null>(null)
  const [draft, setDraft] = useState<Omit<SchoolEvent, 'id'>>(BLANK)
  ...
  const openEdit = (e: SchoolEvent) => { setDraft({ title: e.title, date: e.date, location: e.location, description: e.description, isPast: e.isPast }); setEditing(e); setIsNew(false) }

  const save = () => {
    if (!draft.title.trim()) return showToast('Title is required')
    if (isNew) {
      setEvts(prev => [...prev, { ...draft, id: `e-${Date.now()}` }])
      showToast('Event created ✓')
    } else if (editing) {
      setEvts(prev => prev.map(e => e.id === editing.id ? { ...draft, id: e.id } : e))
      showToast('Event updated ✓')
    }
    setEditing(null); setIsNew(false)
  }

  const del = (id: string) => { setEvts(prev => prev.filter(e => e.id !== id)); setDelConfirm(null); showToast('Event deleted') }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
              {filtered.map(e => (
                <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                  ...
                  <td className="px-5 py-4 text-gray-600 dark:text-gray-300 whitespace-nowrap">{e.date}</td>
      ...
      <Modal open={FORM_OPEN} onClose={() => { setEditing(null); setIsNew(false) }} title={isNew ? 'New Event' : 'Edit Event'}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea rows={3} className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-yellow-400/40 resize-none" value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })} />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={draft.isPast} onChange={e => setDraft({ ...draft, isPast: e.target.checked })} className="h-4 w-4 rounded accent-[#E8B84B]" />
            <span className="text-sm text-gray-700 dark:text-gray-300">Mark as past event</span>
          </label>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={save} className="rounded-xl bg-[#E8B84B] px-5 py-2 text-sm font-semibold text-[#0d1b0d] hover:bg-[#d4a43a]">{isNew ? 'Create' : 'Save'}</button>
          </div>
        </div>
      </Modal>
