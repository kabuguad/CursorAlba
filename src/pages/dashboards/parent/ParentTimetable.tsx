...
import { useParentTimetable, useParentStudentProfile } from '../../../hooks/useParentData'
...
export function ParentTimetable() {
  ...
  const { data: timetable, isLoading } = useParentTimetable()
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">
          {student ? `${student.firstName} ${student.lastName}` : ''} · {classInfo ? `${classInfo.grade} ${classInfo.stream}` : ''} · Term 2, 2026
        </p>
      </div>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[700px]">
          <div className="grid grid-cols-6 gap-2 mb-2">
            <div />
            {DAYS.map(day => (
              <div key={day} className={`rounded-xl px-3 py-2 text-center text-sm font-semibold ${day===todayName?'bg-violet-700 text-white':'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
                {DAY_ABBR[day]}
              </div>
            ))}
          </div>
            ...
            return times.map(time => (
              <div key={time} className="grid grid-cols-6 gap-2 mb-2">
                ...
                {DAYS.map(day => {
                  ...
                  const pIdx = times.indexOf(time)
                  if (!slot) return <div key={day} className="rounded-xl bg-gray-50 dark:bg-gray-800/30 border border-dashed border-gray-200 dark:border-gray-700 min-h-[72px]" />
                  return (
                    <div key={day} className={`rounded-xl p-3 min-h-[72px] ${PERIOD_COLORS[pIdx % PERIOD_COLORS.length]} text-white`}>
                      <p className="text-xs font-bold opacity-90 leading-tight">{slot.subjectName}</p>
                      <div className="mt-1.5 space-y-0.5">
                        <div className="flex items-center gap-1 text-xs opacity-75">
                          <User className="h-2.5 w-2.5" /><span className="truncate">{slot.teacherName.split(' ').pop()}</span>
                        </div>
                        <div className="flex items-center gap-1 text-xs opacity-75">
